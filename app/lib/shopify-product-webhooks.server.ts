...
import {
  markShopifyProductDeletedByGid,
  syncShopifyProductByGid,
} from "./shopify-sync.server";

type ShopifyProductWebhookInput = {
...
export type ShopifyProductWebhookResult = {
...
function getShopifyProductGid(payload: unknown) {
  const record = getPayloadObject(payload);
  const graphqlId = record.admin_graphql_api_id;
  if (typeof graphqlId === "string" && graphqlId.startsWith("gid://shopify/Product/")) {
    return graphqlId;
  }
  ...
  if (typeof id === "number" || typeof id === "string") {
    ...
    if (trimmed) return `gid://shopify/Product/${trimmed}`;
  }

  return null;
}

function minimalProductPayloadMetadata(payload: unknown) {
  const record = getPayloadObject(payload);
  return {
    id: record.id ?? null,
...
async function insertWebhookEvent(
  ...
  const result = await db.query<WebhookEventRow>(
    ...
    [
      ...
      input.webhookId,
      input.resourceGid,
      JSON.stringify(minimalProductPayloadMetadata(input.payload)),
    ],
  );
...
export async function handleShopifyProductWebhook(
  db: QueryExecutor,
  input: ShopifyProductWebhookInput,
  resolveAdmin: ShopifyAdminResolver,
): Promise<ShopifyProductWebhookResult> {
  ...
  const resourceGid = getShopifyProductGid(input.payload);
  ...
  if (!shopDomain) {
    return {
      ...
      eventId: null,
      resourceGid,
      message: "Shopify product webhook did not include a shop domain.",
    };
  }
  ...
  if (!webhookId) {
    return {
      ...
      eventId: null,
      resourceGid,
      message: "Shopify product webhook did not include a webhook ID.",
    };
  }
  ...
  if (!inserted) {
    return {
      ...
      eventId: null,
      resourceGid,
      message: "Duplicate Shopify product webhook ignored.",
    };
  }
  ...
  if (!resourceGid) {
    const message = "Shopify product webhook did not include a usable product resource ID.";
    await markWebhookEvent(db, inserted.id, "failed", message);
    await recordWebhookSystemEvent(
      db,
      installation.tenant_id,
      "Shopify product webhook missing product ID",
      message,
      webhookId,
      { topic, shopDomain },
    );
  ...
  try {
    if (topic === "PRODUCTS_DELETE") {
      const result = await markShopifyProductDeletedByGid(
        db,
        installation.tenant_id,
        resourceGid,
      );
      await recordWebhookSystemEvent(
        ...
        "Shopify deleted a product; Operations Kit marked the local Shopify product link as deleted and preserved operational history.",
        webhookId,
        { topic, shopDomain, resourceGid, ...result },
      );
      ...
      return {
        ...
        eventId: inserted.id,
        resourceGid,
        message: "Shopify product delete webhook processed.",
      };
    }

    const admin = await resolveAdmin(shopDomain);
    await syncShopifyProductByGid(db, installation.tenant_id, admin, resourceGid);
    await markWebhookEvent(db, inserted.id, "processed");
    return {
      status: "processed",
      eventId: inserted.id,
      resourceGid,
      message: "Shopify product webhook processed.",
    };
  } catch (error) {
    const message =
      error instanceof Error ? error.message : "Shopify product webhook processing failed.";
    await markWebhookEvent(db, inserted.id, "failed", message);
    await recordWebhookSystemEvent(
      db,
      installation.tenant_id,
